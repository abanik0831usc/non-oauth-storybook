...
You have the option of sending the response as a `String` or as an `Object`.

To send it as a `String` - you'll require to `stringify` the response:

```
JSON.stringify(idxMessage)
```

To send it as an `Object` - the response can be send as is.
